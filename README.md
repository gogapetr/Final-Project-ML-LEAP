...
logistic regression (Part B) and a small neural network (Part C). Finally, we compare them (Part D)
...
As the classical baseline I used a logistic regression. Previously we splited the data set into training data
(80%) and test data (20%) using stratification and random_state=42. Also, I sorted columns into a  numeric
group and a categorical group. Now I was ready to start processing data.For numeric features process involved
using a median imputation and standard scaling, while for categorical features using most-frequent value
...
set.Finally, I drew the confusion matrix for the classical model.
...
score of 0.6552. The neural network achieved an accuracy score of 0.8560 and a F1
score of 0.6667. F1 is the more informative metric than accuracy, because the higher-income class(>50K) is the minority class.

Overall, the meural network performs better. The neural network is a more complex model that
allowed for a slightly more improved predictive performance. The confusion matrices show that
both models are making a similar number of mistakes. It is noteworthy that the neural network
correctly identifies slightly more cases where people have an income of >50K and makes less False Positives.
...
transperant model is still a strong baseline.

The improvement in F1 indicates that the neural network was able to identify some additional
patterns in the data. Ultimately, the modest improvement is not enough to justify the usage of
such a complex model. Especially, since the current setting values both transparency and
explainability.

Considering the potential applications of the model, it is crucial to underline the fact that the
model's predictions will inform real-world desicions about benefits and outreach. The logistic
regression might perform worse, but the upside is that its decisions are easier to understand and
explain. The difference in predictive performance between logistic regression and neural network
is too small to make using a more complex model better than a simple model.
This conclusion is also supported by the confusion matrices.

### Limitations

1. Imbalanced Dataset: The model is learning mostly from the majority class. Accuracy is important in the
case of a reasonably balanced data set. Accuracy can be misleading here, because the model could be
performing poorly on the minority class.

2. Low F1 scores: In both models, F1 scores are still not enough for deployment in a real-world programme.
Further improvement and validation is needed.

3. Ethical Concerns: A model do not consider matters of fairness or policy questions. Undoubtably, structual
inequalities are reflected in features of the data set (e.g. race, age, gender) and the deployment of a model
trained on such data raises ethical concerns.

## Reflection: what you would improve with more time

If I had more time, I would prioritise addressing the limitations listed above as best as I could. It is
important to improve the model by making it more balanced, more reliable, and more fair. Performance could
benefit from a more balanced data set and precision-recall curves and F1/recall tradeoff analysis. I would
also investigate whether the neural network’s small advantage is consistent or just random. This requires
actions, like a closer look at mean and standard deviation across runs and repeated cross-validation.
Finally, I would also examine closer the results for structual inequalities. For example, whether the model
is systemically missing eligible households or whether certain sesitive features like race, sex and age are
are responsible for unfair predictions.

Overall, the results indicate that the neural network is better, but not decisively so. The logistic
regression is still standing as an attractive and trasparent alternative. Ultimately, both models need
further improvement and refinement before being used in a real desicion-support system.
